...
- Export: `handleExportEventEffect` → `scheduleCaseExportEffect` (awaits coalesced join). `listenForEventsStream` + `Stream.runForEach` is the only LISTEN path. Coalesced; no parallel case writes.
- Startup: `reconcileStaleJobsEffect` / `reconcileStuckPlaybookRunsEffect` keep `DomainTag` in `E`; `reconcileWorkerStartupEffect` catches at the boot edge (log + continue). Fails stale `running` rows (per-Cap expire window); re-advances playbook runs with no open Jobs (`queued`/`running`/`blocked`). All-`blocked` leftover recipes are not this path.
