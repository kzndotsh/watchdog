...
| `ds:ban` | SectionLabel SoT, freestyle palette in domains, opaque-id `.slice`, WD manifest (`shared/ui` excl. `shadcn/` + `__tests__/`), **loading doctrine bans** (RoutePending in routes, domain skeleton imports, … — [`UI.md`](UI.md) § ds:check loading bans) |
...
- Match a route to [`UI.md`](UI.md) § Route patterns — shell-first, one pending surface per region, error granularity via `RegionBoundary`.
- Tables: **`DataTable` `pending` only** — never `PendingRegion` ([§ Tables](UI.md#tables)).
- New `useSuspenseQuery` on a warmed page: update the matching `warm*Queries` helper in the same PR ([`DATA.md`](DATA.md) parity table).
