...
- **Client-safe display:** `@watchdog/core/job-display` exports `capabilityIdLabel`, `playbookIdLabel`, `summarizeJobInput`, `jobActivityLabel` without pulling blob/db/env. Web client code must not import the `@watchdog/core` barrel for these helpers.
