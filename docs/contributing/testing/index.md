...
**Web lib tests run in the component project** (jsdom), not `pnpm test:unit`. Unit is packages + worker + CLI. Vitest 5 clears mock call history before each test. `vitest.config.ts` sets `clearMocks: false` so assertions on calls made at import time or in `beforeAll` still see that history.
