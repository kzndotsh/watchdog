...
Playwright covers the core loop (Collect → Harvest → Triage Accept → Identifiers), custody Accept gates, route smoke, auth sign-up, team invite accept, instance-admin Users tab, case create, Collect paste, and Triage reject. Lib tests pin Collect queue filters, jobs domain detail/run-input/status, Dossier confirmed-evidence + claim form, Triage filters/evidence/decide-header + Accept disable (`TriagePatchBody`), intake evidence helpers, Tasks due-date + form, connection table writes (`unverified` / `related_to` notes), identifier commit (handle-without-platform), paste error aliases, dashboard selectors, case overview activity, jump-nav, queue-selection, prefetch **`warm*Queries`**, **`listPending`**, `slugifyName`, and display helpers (`formatOpaqueId` / `group-by-day`). Component: Accept gate copy, bulk-add preview, Triage Accept disable, **`RegionBoundary`**, **`LoadingRegion`**, **`DataTable` pending**, skeletons. Remaining smoke below is layout, live Query/SSE, vault Settings, and chrome that has no lib contract.
