...
5. **Tasks**: `/tasks` kanban only: New task / lane quick-create; drag across columns (status) and within a column (order persists); edit dialog (title, priority, date-only due, entity); Dossier Tasks tab = entity-scoped board; Task is not a Graph write.
