...
| `EvidencePicker` | Dense multi-select Case Evidence (chip-height Add/+ · checklist popover; `layout="panel"` for parent shells; options as `readonly EvidenceOption[]`) | Dossier composers · identifier Link · Triage | Job cite display | `EvidenceCiteChips` | **domain** (`dossier/components/evidence-picker.tsx` re-exports `shared/ui`) | no | : |
