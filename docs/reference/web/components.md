...
| `DetailStatusChip` | Outcome / tag pill: same outline + `CHIP_SIZE_CLASS` as VocabBadge (IdChip height/radius; `text-label-meta`); exports `CHIP_SIZE_CLASS` / `DETAIL_CHIP_CLASS` for skeleton shape parity | Identifier evidence preview · Triage patch-op warnings · table/composer tags | Detail context strips (use plain `span`s + `StatusInk` for lifecycle) | `StatusBadge` · `KindBadge` | canonical | yes | : |
