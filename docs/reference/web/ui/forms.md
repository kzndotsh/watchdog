...
This page defines the boundary between TanStack Form and local state.

## Form library

Use `@tanstack/react-form` only. Do not add `react-hook-form`.
...
- Confirmed↔evidence gate + copy: `shared/lib/confirmed-evidence.ts` (also Triage + connection dialog).
