...
- **DataTable Actions column**: trailing ~48px; `RowActionsMenu` trigger stays a `button` (row-click ignore list). `DataTable` body rows include `group` so hover-reveal works. Delete last + separated.
