...
| `settings` | Settings page (sidebar: Account / Security / Appearance / Team / Users / API Keys / Credentials) | `/settings` (`?tab=`) | `settings-shell`, `settings-appearance-panel`, `settings-credentials-form` (+ `settings-credentials-handlers` bind* helpers); Team tab is `auth/ui/team`; Users tab is `auth/ui/users` (instance admin only) | `settings.functions.ts` + `types.ts` + `queries.ts` |
...
| `search` | Shell Mod+K command palette + Shortcuts sheet + inset ContextMenu chrome | : (shell chrome) | `search-chrome.tsx` (`SearchUiContext` + `AppInsetContextMenu`), `command-palette.tsx` (Jump to + idle Commands + `searchCase`), `hooks/use-search-ui` | `search.functions.ts` + `types.ts` + `queries.ts` |
...
| Intake actions (Collect) | `useIntakeActions` — Harvest/Extract/Enrich/Hide/Restore/Attach: errors → `actionError` / Collect `FormInlineError`; no success toast (job queue + detail are feedback). Dump (file/paste/URL) via `useDumpEvidence`: `toast.promise` for uploads + success toasts for paste/URL. |
