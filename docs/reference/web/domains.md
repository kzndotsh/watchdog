...
| `search` | Shell Mod+K command palette + Shortcuts sheet + inset ContextMenu chrome. Jump destinations follow sidebar nav and omit `/ui`. | : (shell chrome) | `search-chrome.tsx` (`SearchUiContext` + `AppInsetContextMenu`), `command-palette.tsx` (Jump to + idle Commands + `searchCase`), `hooks/use-search-ui` | `search.functions.ts` + `types.ts` + `queries.ts` |
